...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['consul_paci'])) {
    // Verifica se o campo nome_paciente foi enviado
    if (isset($_POST['nome_paciente'])) {
        ...
        try {
            ...
            // Consulta SQL
            $stmt = $conn->prepare("SELECT * FROM anamnese WHERE nome_completo LIKE :nome LIMIT 1");
...
background-position: center calc(100% + 100px);  /* 50px acima do rodapé */   
